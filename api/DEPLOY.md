...
TypeScript compilation and production `npm ci` run in CI. Dependencies are shipped as `node_modules.tar.gz`. The droplet only extracts them, runs migrations, and `systemctl restart`.
...
- On push to `main` (or manual **workflow_dispatch**), if those steps pass, Actions rsyncs `dist/`, `node_modules.tar.gz`, lockfiles, `scripts/migrate.mjs`, and `migrations/` to `DEPLOY_PATH`, then runs `./deploy/activate.sh` over SSH (extract deps, migrate, `systemctl restart book-camera-api`).
